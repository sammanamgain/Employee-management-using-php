...
if (isset($_POST['submit'])) {
    ...
    if (empty($_POST['url'])) {
        ...
        $address = htmlspecialchars($_POST['url']);
    }
    ...
    if (empty($errors['name']) && empty($errors['address'])) {
        ...
        //storing into db
        $sql = "INSERT INTO companydetails(Name,Address,url) VALUES('$company_name','$address','$url')";
...
<form method="post" action="form.php">
  <div class="mb-3">
    ...
    <label for="exampleInputEmail1" class="form-label">URL</label>
        <div> <?php echo "<p> {$errors['address']} </p>" ?></div>
    <input  name="url" type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo  $address;?>">
